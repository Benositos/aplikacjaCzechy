        ...
        child: ListView(
          ...
          children: [
            ...
            _SwitchRow(
              ...
              subtitle:
                  'Nudges at 75%, 100%, 150% and 200% of your daily step goal.',
...
/// Time zone selection: Phone / GPS / Custom.
class _TimezoneBlock extends ConsumerStatefulWidget {
  const _TimezoneBlock();

  @override
  ConsumerState<_TimezoneBlock> createState() => _TimezoneBlockState();
}

class _TimezoneBlockState extends ConsumerState<_TimezoneBlock> {
  bool _detecting = false;

  static const _commonTimezones = <String>[
    'UTC',
    'Europe/Warsaw',
    'Europe/Lisbon',
    'Europe/London',
  ...
  String _offsetLabel(String tzId) {
    try {
  ...
  /// Day-keyed caches (steps, app_usage, milestone_fired) were written under
  /// the previous timezone's midnight. After a tz switch their lookup keys no
  /// longer match — wipe them so the next refresh repopulates cleanly.
  Future<void> _wipeCachesAfterTzChange() async {
    final db = ref.read(databaseProvider);
    await db.wipeTimezoneScopedCaches();
  }
  ...
  Future<void> _detectGps() async {
    ...
    final result = await svc.detectTimezone();
    if (!mounted) return;
    setState(() => _detecting = false);

    if (result.isSuccess) {
      final repo = ref.read(profileRepositoryProvider);
      await repo.setCustomTimezone(result.timezoneId);
      await repo.setTimezoneMode(1);
      await _wipeCachesAfterTzChange();
      if (!mounted) return;
      _snack('Detected: ${result.timezoneId} (UTC${_offsetLabel(result.timezoneId!)})');
    } else {
      _snack(switch (result.error!) {
        LocationResult.servicesDisabled =>
          'Location services are off — enable in phone Settings.',
        LocationResult.permissionDenied =>
          'Location permission denied — grant in phone Settings.',
        LocationResult.timeout =>
          "Couldn't get a GPS fix in time — try again outdoors.",
        LocationResult.resolveFailed =>
          "Got a location but couldn't resolve its timezone.",
      });
    }
  }
  ...
  Future<void> _pickCustom() async {
    ...
    final repo = ref.read(profileRepositoryProvider);
    await repo.setCustomTimezone(picked);
    await repo.setTimezoneMode(2);
    await _wipeCachesAfterTzChange();
    if (!mounted) return;
    _snack('Time zone: $picked (UTC${_offsetLabel(picked)})');
  }

  Future<void> _selectPhone() async {
    final repo = ref.read(profileRepositoryProvider);
    await repo.setTimezoneMode(0);
    await repo.setCustomTimezone(null);
    await _wipeCachesAfterTzChange();
  }
  ...
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final profile = ref.watch(profileProvider).valueOrNull;
    final mode = profile?.timezoneMode ?? 0;
    final customId = profile?.customTimezoneId;

    final detailLine = switch (mode) {
      0 => 'Using device time zone (${DateTime.now().timeZoneName})',
      1 when customId != null =>
        'Detected: $customId (UTC${_offsetLabel(customId)})',
      2 when customId != null =>
        'Selected: $customId (UTC${_offsetLabel(customId)})',
      _ => 'No time zone set — falling back to device.',
    };
    ...
    return Column(
      ...
      children: [
        Row(
          children: [
            Expanded(child: _Chip(label: 'Phone', selected: mode == 0, onTap: _selectPhone)),
            const SizedBox(width: AppTheme.space2),
            Expanded(child: _Chip(label: 'GPS', selected: mode == 1, onTap: _detectGps)),
            const SizedBox(width: AppTheme.space2),
            Expanded(child: _Chip(label: 'Custom', selected: mode == 2, onTap: _pickCustom)),
          ],
        ),
        const SizedBox(height: AppTheme.space3),
        Container(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(detailLine, style: theme.textTheme.bodyMedium),
              if (mode == 1) ...[
                ...
                Row(
                  children: [
                    OutlinedButton(
                      ...
                      child: Text(_detecting ? 'Detecting…' : 'Detect now'),
                    ),
                  ],
                ),
              ],
        ...
        const SizedBox(height: AppTheme.space2),
        Text(
          'Switching time zones clears cached data — Steps and Focus will refresh fresh in the new zone.',
...
class _TimezonePickerSheet extends StatelessWidget {
  ...
  final String Function(String) offsetLabel;
    ...
    return SafeArea(
      child: Container(
        ...
        child: Column(
          ...
          children: [
            Text('Pick a time zone', style: theme.textTheme.titleLarge),
            const SizedBox(height: AppTheme.space3),
            Expanded(
              child: ListView.builder(
                itemCount: timezones.length,
                itemBuilder: (context, i) {
                  final id = timezones[i];
                  return ListTile(
                    title: Text(id, style: theme.textTheme.bodyLarge),
                    trailing: Text(
                      'UTC${offsetLabel(id)}',
                      style: AppTextStyles.mono(fontSize: 12).copyWith(
